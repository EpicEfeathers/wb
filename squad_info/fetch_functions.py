...
async def get_squad_uids(session, squad_name: str) -> list:
    response = await fetch(session, f"https://wbapi.wbpjs.com/squad/getSquadMembers?squadName={squad_name}")

    try:
        uids = [user["uid"] for user in response]
        return uids
    except Exception as e:
        raise Exception(f"[ERROR] Failed to fetch squad members for squad '{squad_name}' \n\nException: {e}")


async def get_user_data(session, uid: str) -> dict:
    response = await fetch(session, f"https://wbapi.wbpjs.com/players/getPlayer?uid={uid}")
